...
describe('wallet connect flow', () => {
  ...
  it('forgets the address on disconnect without an SDK call', async () => {
    ...
    // Looked up by role alone, not accessible name: once connected the
    // button's visible text is the truncated address (its `title` carries
    // the full address, but title only feeds the accessible name when
    // there's no visible text content — here there is), so matching by
    // the "connect wallet" name would no longer find it.
    await waitFor(() => {
      expect(screen.getByRole('button')).toHaveTextContent('GAAA…AAAA');
    });
