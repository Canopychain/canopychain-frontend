...
COPY --from=build --chown=canopychain:nodejs /app/.next/standalone ./
COPY --from=build --chown=canopychain:nodejs /app/.next/static ./.next/static
COPY --from=build --chown=canopychain:nodejs /app/public ./public
